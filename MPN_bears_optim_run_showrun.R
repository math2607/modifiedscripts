MPN_bears_optim_run_showrun <- function (BioGeoBEARS_run_object = define_BioGeoBEARS_run(), 
          skip_optim = FALSE, skip_optim_option = "return_loglike") 
{
  ...
  m = NULL
  if (is.null(BioGeoBEARS_run_object$min_branchlength) == 
      FALSE) {
  ...
  if (is.null(BioGeoBEARS_run_object$states_list) == FALSE) {
    if (is.na(BioGeoBEARS_run_object$states_list[[1]]) == 
        FALSE) {
  ...
  if (.Platform$GUI != "AQUA" && ((is.na(num_cores_to_use) == 
    ...
    cat("\nYour computer has ", num_cores_computer_has, 
        " cores. You have chosen to use:\nnum_cores_to_use = ", 
  ...
  if ((is.numeric(BioGeoBEARS_run_object$timeperiods))) {
    ...
    model_results = MPN_calc_loglike_sp_stratified_showrun(tip_condlikes_of_data_on_each_state, 
    ...
    calc_ancprobs = BioGeoBEARS_run_object$calc_ancprobs
    model_results = MPN_calc_loglike_for_optim_showrun(params = params, 
                                           BioGeoBEARS_run_object = BioGeoBEARS_run_object, 
                                           phy = phy, tip_condlikes_of_data_on_each_state = tip_condlikes_of_data_on_each_state, 
                                           print_optim = BioGeoBEARS_run_object$print_optim, 
                                           areas_list = areas_list, states_list = states_list, 
